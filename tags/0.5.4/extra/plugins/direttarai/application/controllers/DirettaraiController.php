...
class DirettaraiController extends X_Controller_Action
{

	/**
	 * @var X_VlcShares_Plugins_DirettaRai
	 */
	...
	function init() {
		...
		parent::init();
		if ( !X_VlcShares_Plugins::broker()->isRegistered('direttarai') ) {
			/*
			$this->_helper->flashMessenger(X_Env::_('err_pluginnotregistered') . ": youtube");
			$this->_helper->redirector('index', 'manage');
			*/
			throw new Exception(X_Env::_('err_pluginnotregistered') . ": direttarai");
		} else {
			$this->plugin = X_VlcShares_Plugins::broker()->getPlugins('direttarai');
		}
	}
