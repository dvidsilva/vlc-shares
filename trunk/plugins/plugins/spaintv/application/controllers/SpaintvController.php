...
class SpaintvController extends X_Controller_Action
{

	/**
	 * @var X_VlcShares_Plugins_Spaintv
	 */
	...
	function init() {
		...
		parent::init();
		if ( !X_VlcShares_Plugins::broker()->isRegistered('spaintv') ) {
			/*
			$this->_helper->flashMessenger(X_Env::_('err_pluginnotregistered') . ": youtube");
			$this->_helper->redirector('index', 'manage');
			*/
			throw new Exception(X_Env::_('err_pluginnotregistered') . ": spaintv");
		} else {
			$this->plugin = X_VlcShares_Plugins::broker()->getPlugins('spaintv');
		}
	}
	...
	function proxyAction() {
		...
		if ( $videoUrl === false /*|| $refererUrl === false */) {
			// invalid request
			throw new Exception(X_Env::_('p_spaintv_err_invalidrequest'));
		}
		
		$videoUrl = X_Env::decode($videoUrl);
		//$refererUrl = X_Env::decode($refererUrl);
		
		if ( !defined("X_VlcShares_Plugins_Spaintv::C_$videoUrl") ) {
			throw new Exception(X_Env::_('p_spaintv_err_invalidchannel'));
		}
		
		$videoUrl = constant("X_VlcShares_Plugins_Spaintv::C_$videoUrl");
		
		
		//$userAgent = $this->plugin->config('hide.useragent', true) ? 'User-Agent: vlc-shares/'.X_VlcShares::VERSION.' spaintv/'.X_VlcShares_Plugins_Spaintv::VERSION : 'User-Agent: Mozilla/5.0 (X11; Linux i686; rv:2.0.1) Gecko/20101019 Firefox/4.0.1';
		//$userAgent = 'User-Agent: vlc-shares/'.X_VlcShares::VERSION.' spaintv/'.X_VlcShares_Plugins_Spaintv::VERSION; 
