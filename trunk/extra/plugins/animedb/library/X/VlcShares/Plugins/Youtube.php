...
/**
 * Add Youtube library site as a video source
 * @author ximarx
 *
 */
class X_VlcShares_Plugins_Youtube extends X_VlcShares_Plugins_Abstract implements X_VlcShares_Plugins_ResolverInterface {
	...
	function gen_beforeInit(Zend_Controller_Action $controller) {
		...
		$this->helpers()->registerHelper('youtube', new X_VlcShares_Plugins_Helper_Megavideo());
	}
	...
	public function preGetModeItems($provider, $location, Zend_Controller_Action $controller) {
		...
		if ( $url ) {
			...
			$link = new X_Page_Item_PItem('megavideo-warning', X_Env::_('p_youtube_invalidmegavideo'));
	...
	public function getIndexMessages(Zend_Controller_Action $controller) {
		...
		if ( $this->config('auth.username', '') == '' || $this->config('auth.password', '') == '' ) {
			$m = new X_Page_Item_Message($this->getId(), X_Env::_('p_youtube_dashboardwarning'));
	 ...
	 *  - check for data/youtube/ path writable
	...
	public function getTestItems(Zend_Config $options,Zend_Controller_Action $controller) {
		...
		$test = new X_Page_Item_Test($this->getId().'-writeaccess', '[youtube] Checking for write access to /data/youtube/ folder');
		if ( is_writable(APPLICATION_PATH . '/../data/youtube/') ) {
		...
		$test = new X_Page_Item_Test($this->getId().'-credentials', '[youtube] Checking for authentication credentials');
		if ( $this->config('auth.username', '') != '' && $this->config('auth.password', '') != '' ) {
			$test->setType(X_Page_Item_Message::TYPE_INFO);
	...
	private function _fetchLists(X_Page_ItemList_PItem $items) {
		...
		$pageIndex = $this->config('index.url', 'http://youtube.tv/forum/liste.php');
		$htmlString = $this->_loadPage($pageIndex);
		$dom = new Zend_Dom_Query($htmlString);
		
		// xpath index stars from 1
		$results = $dom->queryXpath('//div[@class="blockrow"]/div[@class="forumbits"][1]//div[@class="forumrow"]//a[@rel="nofollow"]');
	...
	private function _fetchThreads(X_Page_ItemList_PItem $items, $category, $letter) {
		...
		$baseUrl = $this->config('base.url', 'http://youtube.tv/forum/');
	...
	private function _fetchVideos(X_Page_ItemList_PItem $items, $category, $letter, $thread) {
		...
		for ( $i = 0; $i < $results->count(); $i++, $results->next()) {
			...
			$label = trim(trim($current->nodeValue), chr(0xC2).chr(0xA0));
			if ( $label == '') {
				$label = X_Env::_('p_youtube_nonamevideo');
			}
			$href = $current->getAttribute('href');
		...
		/*
		if (!$found) {
			$item = new X_Page_Item_PItem($this->getId().'-ops', X_Env::_('p_youtube_opsnovideo'));
	...
	private function _loadPage($uri, $forceAuth = false) {
		...
		$jarFile = APPLICATION_PATH . '/../data/youtube/cookie.jar';
		...
		if ( $forceAuth && $this->config('auth.username', '') != '' && $this->config('auth.password', '') != '' && !$this->_isAuthenticated($htmlString) ) {
			...
			$pageLogin = $this->config('login.url', 'http://youtube.tv/forum/login.php?do=login');
	 ...
	 * @param string $htmlString an youtube page
	 */
